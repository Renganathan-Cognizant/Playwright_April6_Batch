...
test("simple web table handling Method1", async({page})=>{
    ...
    const cols = rows.first().locator("td") //4 coloumns in 1st row
...
async function inputChecker(rows: Locator, page: Page, name: string) {
    const nameMatch = rows.filter({
        has: page.locator("td"), //#simpletable tbody tr td //12
        hasText: name
    });
...
test("web table - calculation", async({page})=>{
    ...
    const rowsCount = await tbody.locator("tr").count(); //4 #Shopping tbody tr
